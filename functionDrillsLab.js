...
console.log(jsNinja()) // Console log check 
////////////////// PROBLEM 3 ////////////////////
/*
...
console.log(exclaimThree('Jatou')) // When calling function, console log function name (argument value)
...
function bigOrSmall(arr){
  ...
  for (let i = 0; i <= bigOrSmallArray.length; i++){
    if(bigOrSmallArray[i] > 100){
      answers.push('big') // use dot push to add elements into array 
    }else{
      answers.push('small')
    }
  }
...
function emailCheck(email){
  let changeToString = String(email);
  let trimString = changeToString.trim()
...
//CODE HERE
function buyFrogs(amount){
return (amount/3)
}
buyFrogs(40)
let totalFrogs = buyFrogs(40)
...
function numArr(arr){
  for (i=0; i < sampleArray.length; i++){
    if(sampleArray[i] < Math.max(sampleArray)){
      return('true')
    }else{
      return('false')
  }
